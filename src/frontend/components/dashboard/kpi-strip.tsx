...
export function KpiStrip({ summary }: { summary: DashboardSummary }) {
  return (
    <div className="mb-6 grid grid-cols-2 gap-2.5 sm:gap-4 md:grid-cols-3 xl:grid-cols-5">
      ...
      <KpiCard label="Total Stasiun" value={formatNumber(summary.totalStations)} detail="Site terpantau" icon={Building2} colorClass="bg-blue-50 text-dashboard-primary" />
      ...
      <div className="col-span-2 sm:col-span-1">
        <KpiCard label="Warning" value={formatNumber(summary.warningSites + summary.criticalSites)} detail="Purity, tekanan, status" icon={TriangleAlert} colorClass="bg-amber-50 text-dashboard-warning" />
      </div>
    </div>
  );
}
